...
import com.goatedchat.mesh.services.SeenMessageStore
import com.goatedchat.mesh.ui.ChatState
import com.goatedchat.mesh.ui.MeshDelegateHandler
import com.goatedchat.mesh.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.goatedchat.mesh.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (!content.startsWith("goatedchat1:")) return@launch

                val base64Content = content.removePrefix("goatedchat1:")
                ...
                if (packet.type != com.goatedchat.mesh.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.goatedchat.mesh.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                if (file != null) {
                    ...
                    val savedPath = com.goatedchat.mesh.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = BitchatMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
