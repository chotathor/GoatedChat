package com.goatedchat.mesh.nostr

import android.content.Context
import android.util.Log
import com.goatedchat.mesh.model.ReadReceipt
...
class NostrTransport(
    ...
    companion object {
        ...
        private const val READ_ACK_INTERVAL = com.goatedchat.mesh.util.AppConstants.Nostr.READ_ACK_INTERVAL_MS // ~3 per second (0.35s interval like iOS)
    ...
    fun sendPrivateMessage(
        ...
        transportScope.launch {
            try {
                ...
                // Strict: lookup the recipient's current GoatedChat peer ID using favorites mapping
                val recipientPeerIDForEmbed = try {
                    com.goatedchat.mesh.favorites.FavoritesPersistenceService.shared
                ...
                val embedded = NostrEmbeddedGoatedChat.encodePMForNostr(
                    content = content,
                    messageID = messageID,
    ...
    private fun sendNextReadAck() {
        ...
        transportScope.launch {
            try {
                ...
                val ack = NostrEmbeddedGoatedChat.encodeAckForNostr(
    ...
    fun sendDeliveryAckGeohash(
        ...
        transportScope.launch {
            try {
                ...
                val embedded = NostrEmbeddedGoatedChat.encodeAckForNostrNoRecipient(
                    type = NoisePayloadType.DELIVERED,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                )
    ...
    fun sendPrivateMessageGeohash(
        ...
        val geohash = sourceGeohash ?: run {
            val selected = try {
                com.goatedchat.mesh.geohash.LocationChannelManager.getInstance(context).selectedChannel.value
            } catch (_: Exception) { null }
            if (selected !is com.goatedchat.mesh.geohash.ChannelID.Location) {
        ...
        transportScope.launch {
            try {
                ...
                // Build embedded GoatedChat packet without recipient peer ID
                val embedded = NostrEmbeddedGoatedChat.encodePMForNostrNoRecipient(
                    content = content,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                ) ?: run {
    ...
    private fun resolveNostrPublicKey(peerID: String): String? {
        try {
            ...
            com.goatedchat.mesh.favorites.FavoritesPersistenceService.shared.findNostrPubkeyForPeerID(peerID)?.let { return it }

            // 2) Legacy path: resolve by noise public key association
            val noiseKey = hexStringToByteArray(peerID)
            val favoriteStatus = com.goatedchat.mesh.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
            ...
            if (peerID.length == 16) {
                val fallbackStatus = com.goatedchat.mesh.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
