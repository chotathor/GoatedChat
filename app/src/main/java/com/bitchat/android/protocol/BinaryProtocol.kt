package com.goatedchat.mesh.protocol
...
data class BitchatPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = BitchatPacket(
            ...
            route = route,
            ttl = com.goatedchat.mesh.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
...
object BinaryProtocol {
    ...
    private fun decodeCore(raw: ByteArray): BitchatPacket? {
        try {
            ...
            if (payloadLength > com.goatedchat.mesh.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH.toUInt()) {
                Log.w("BinaryProtocol", "Payload length ${payloadLength} exceeds maximum allowed (${com.goatedchat.mesh.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH})")
                return null
            }
