...
import com.goatedchat.mesh.onboarding.BluetoothCheckScreen
import com.goatedchat.mesh.onboarding.BluetoothStatus
import com.goatedchat.mesh.onboarding.BluetoothStatusManager
import com.goatedchat.mesh.onboarding.BatteryOptimizationManager
import com.goatedchat.mesh.onboarding.BatteryOptimizationPreferenceManager
import com.goatedchat.mesh.onboarding.BatteryOptimizationScreen
import com.goatedchat.mesh.onboarding.BatteryOptimizationStatus
import com.goatedchat.mesh.onboarding.BackgroundLocationPermissionScreen
import com.goatedchat.mesh.onboarding.InitializationErrorScreen
import com.goatedchat.mesh.onboarding.InitializingScreen
import com.goatedchat.mesh.onboarding.LocationCheckScreen
import com.goatedchat.mesh.onboarding.LocationStatus
import com.goatedchat.mesh.onboarding.LocationStatusManager
import com.goatedchat.mesh.onboarding.OnboardingCoordinator
import com.goatedchat.mesh.onboarding.OnboardingState
import com.goatedchat.mesh.onboarding.PermissionExplanationScreen
import com.goatedchat.mesh.onboarding.PermissionManager
import com.goatedchat.mesh.ui.ChatScreen
import com.goatedchat.mesh.ui.ChatViewModel
import com.goatedchat.mesh.ui.OrientationAwareActivity
import com.goatedchat.mesh.ui.theme.BitchatTheme
import com.goatedchat.mesh.wifiaware.WifiAwareController
import com.goatedchat.mesh.nostr.PoWPreferenceManager
...
class MainActivity : OrientationAwareActivity() {
    ...
    private val forceFinishReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: android.content.Context, intent: android.content.Intent) {
            if (intent.action == com.goatedchat.mesh.util.AppConstants.UI.ACTION_FORCE_FINISH) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val filter = android.content.IntentFilter(com.goatedchat.mesh.util.AppConstants.UI.ACTION_FORCE_FINISH)
        if (android.os.Build.VERSION.SDK_INT >= 33) {
            registerReceiver(
                forceFinishReceiver,
                filter,
                com.goatedchat.mesh.util.AppConstants.UI.PERMISSION_FORCE_FINISH,
                null,
        ...
        com.goatedchat.mesh.service.AppShutdownCoordinator.cancelPendingShutdown()
        ...
        meshService = com.goatedchat.mesh.service.MeshServiceHolder.getOrCreate(applicationContext)
        unifiedMeshService = com.goatedchat.mesh.service.MeshServiceHolder.getUnifiedOrCreate(applicationContext)
    ...
    private fun checkBluetoothAndProceed() {
        ...
        val bleRequired = try { com.goatedchat.mesh.ui.debug.DebugPreferenceManager.getBleEnabled(true) } catch (_: Exception) { true }
        if (!bleRequired) {
        ...
        lifecycleScope.launch {
            ...
            if (permissionManager.isFirstTimeLaunch()) {
                ...
                if (permissionManager.needsBackgroundLocationPermission() &&
                    ...
                    !com.goatedchat.mesh.onboarding.BackgroundLocationPreferenceManager.isSkipped(this@MainActivity)
                ) {
                    mainViewModel.updateOnboardingState(OnboardingState.BACKGROUND_LOCATION_EXPLANATION)
                } else {
    ...
    private fun handleLocationEnabled() {
        ...
        com.goatedchat.mesh.wifiaware.WifiAwareController.startIfPossible()
        checkBatteryOptimizationAndProceed()
    }
    ...
    private fun handleOnboardingComplete() {
        ...
        val bleRequired2 = try { com.goatedchat.mesh.ui.debug.DebugPreferenceManager.getBleEnabled(true) } catch (_: Exception) { true }
        when {
    ...
    private fun startMeshForegroundServiceBestEffort() {
        ...
        try {
            com.goatedchat.mesh.service.MeshForegroundService.start(applicationContext)
    ...
    private fun initializeApp() {
        ...
        lifecycleScope.launch {
            try {
                ...
                com.goatedchat.mesh.nostr.LocationNotesInitializer.initialize(this@MainActivity)
    ...
    private fun handleNotificationIntent(intent: Intent) {
        val shouldOpenPrivateChat = intent.getBooleanExtra(
            com.goatedchat.mesh.ui.NotificationManager.EXTRA_OPEN_PRIVATE_CHAT, 
            false
        )
        
        val shouldOpenGeohashChat = intent.getBooleanExtra(
            com.goatedchat.mesh.ui.NotificationManager.EXTRA_OPEN_GEOHASH_CHAT,
            false
        )
        
        when {
            shouldOpenPrivateChat -> {
                val peerID = intent.getStringExtra(com.goatedchat.mesh.ui.NotificationManager.EXTRA_PEER_ID)
                val senderNickname = intent.getStringExtra(com.goatedchat.mesh.ui.NotificationManager.EXTRA_SENDER_NICKNAME)
            ...
            shouldOpenGeohashChat -> {
                val geohash = intent.getStringExtra(com.goatedchat.mesh.ui.NotificationManager.EXTRA_GEOHASH)
                ...
                if (geohash != null) {
                    ...
                    val level = when (geohash.length) {
                        7 -> com.goatedchat.mesh.geohash.GeohashChannelLevel.BLOCK
                        6 -> com.goatedchat.mesh.geohash.GeohashChannelLevel.NEIGHBORHOOD
                        5 -> com.goatedchat.mesh.geohash.GeohashChannelLevel.CITY
                        4 -> com.goatedchat.mesh.geohash.GeohashChannelLevel.PROVINCE
                        2 -> com.goatedchat.mesh.geohash.GeohashChannelLevel.REGION
                        else -> com.goatedchat.mesh.geohash.GeohashChannelLevel.CITY // Default fallback
                    }
                    val geohashChannel = com.goatedchat.mesh.geohash.GeohashChannel(level, geohash)
                    val channelId = com.goatedchat.mesh.geohash.ChannelID.Location(geohashChannel)
    ...
    private fun handleVerificationIntent(intent: Intent) {
        ...
        if (uri.scheme != "goatedchat" || uri.host != "verify") return
