...
class SecurityManager(private val encryptionService: EncryptionService, private val myPeerID: String) {
    ...
    companion object {
        ...
        private const val MESSAGE_TIMEOUT = com.goatedchat.mesh.util.AppConstants.Security.MESSAGE_TIMEOUT_MS // 5 minutes (same as iOS)
        private const val CLEANUP_INTERVAL = com.goatedchat.mesh.util.AppConstants.Security.CLEANUP_INTERVAL_MS // 5 minutes
        private const val MAX_PROCESSED_MESSAGES = com.goatedchat.mesh.util.AppConstants.Security.MAX_PROCESSED_MESSAGES
        private const val MAX_PROCESSED_KEY_EXCHANGES = com.goatedchat.mesh.util.AppConstants.Security.MAX_PROCESSED_KEY_EXCHANGES
    }
    ...
    fun validatePacket(packet: BitchatPacket, peerID: String): Boolean {
        ...
        if (processedMessages.contains(messageID)) {
            ...
            val isFreshAnnounce = messageType == MessageType.ANNOUNCE &&
                    packet.ttl >= com.goatedchat.mesh.util.AppConstants.MESSAGE_TTL_HOPS
    ...
    private fun verifyPacketSignature(packet: BitchatPacket, peerID: String): Boolean {
        try {
            ...
            if (MessageType.fromValue(packet.type) == MessageType.ANNOUNCE) {
                ...
                try {
                    val announcement = com.goatedchat.mesh.model.IdentityAnnouncement.decode(packet.payload)
