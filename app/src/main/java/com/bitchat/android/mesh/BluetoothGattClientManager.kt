...
import com.goatedchat.mesh.util.AppConstants
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch
import java.util.*
import kotlinx.coroutines.Job
import com.goatedchat.mesh.ui.debug.DebugSettingsManager
import com.goatedchat.mesh.ui.debug.DebugScanResult
...
class BluetoothGattClientManager(
    ...
    private fun isClientRoleEnabled(): Boolean {
        return isBleTransportEnabled() &&
            (try { com.goatedchat.mesh.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true })
    }
    ...
    private fun handleScanResult(result: ScanResult) {
        ...
        val dbg = try { com.goatedchat.mesh.ui.debug.DebugSettingsManager.getInstance() } catch (_: Exception) { null }
    ...
    private fun connectToDevice(device: BluetoothDevice, rssi: Int, peerID: String? = null) {
        ...
        val deviceAddress = device.address
        Log.i(TAG, "Connecting to goatedchat device: $deviceAddress (peerID: $peerID)")
