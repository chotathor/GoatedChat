...
class UnifiedMeshService(
    ...
    override fun sendFavoriteNotification(peerID: String, isFavorite: Boolean) {
        val myNpub = try {
            com.goatedchat.mesh.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)?.npub
        } catch (_: Exception) {
            null
        }
    ...
    private fun mergedPeerIDs(): List<String> {
        ...
        try { ids.addAll(com.goatedchat.mesh.services.AppStateStore.peers.value) } catch (_: Exception) { }
