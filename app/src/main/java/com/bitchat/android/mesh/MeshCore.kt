...
import com.goatedchat.mesh.model.PrivateMessagePacket
...
class MeshCore(
    ...
    private fun setupDelegates() {
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                try { com.goatedchat.mesh.services.AppStateStore.setTransportPeers(transport.id, peerIDs) } catch (_: Exception) { }
    ...
    fun sendVerifyChallenge(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val payload = NoisePayload(
            ...
            data = com.goatedchat.mesh.services.VerificationService.buildVerifyChallenge(noiseKeyHex, nonceA)
        )
        sendNoisePayloadToPeer(payload, peerID)
    }

    fun sendVerifyResponse(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val tlv = com.goatedchat.mesh.services.VerificationService.buildVerifyResponse(noiseKeyHex, nonceA) ?: return
        val payload = NoisePayload(
    ...
    private fun buildAnnouncementPayload(announcement: IdentityAnnouncement, nickname: String): ByteArray? {
        ...
        try {
            if (directPeersForGossip.isNotEmpty()) {
                tlvPayload += com.goatedchat.mesh.services.meshgraph.GossipTLV.encodeNeighbors(directPeersForGossip)
            }
    ...
    private fun getDirectPeerIDsForGossip(): List<String> {
        return try {
            ...
            try { com.goatedchat.mesh.services.AppStateStore.setTransportDirectPeers(transport.id, localDirect) } catch (_: Exception) { }
            val union = try {
                com.goatedchat.mesh.services.AppStateStore.getDirectPeers().ifEmpty { localDirect }
        ...
        return try {
            val withRoute = try {
                ...
                if (recipient != null && !recipient.contentEquals(SpecialRecipients.BROADCAST)) {
                    ...
                    val path = com.goatedchat.mesh.services.meshgraph.RoutePlanner.shortestPath(myPeerID, destination)
