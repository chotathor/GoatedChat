...
import com.goatedchat.mesh.sync.PacketIdUtil
...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket) {
        ...
        try {
            ...
            val noisePayload = com.goatedchat.mesh.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                com.goatedchat.mesh.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    // Decode TLV private message exactly like iOS
                    val privateMessage = com.goatedchat.mesh.model.PrivateMessagePacket.decode(noisePayload.data)
                ...
                com.goatedchat.mesh.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = com.goatedchat.mesh.model.BitchatFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "🔓 Decrypted encrypted file from $peerID: name='${file.fileName}', size=${file.fileSize}, mime='${file.mimeType}'")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = com.goatedchat.mesh.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = BitchatMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = com.goatedchat.mesh.features.file.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                ...
                com.goatedchat.mesh.model.NoisePayloadType.DELIVERED -> {
                ...
                com.goatedchat.mesh.model.NoisePayloadType.READ_RECEIPT -> {
                ...
                com.goatedchat.mesh.model.NoisePayloadType.VERIFY_CHALLENGE -> {
                ...
                com.goatedchat.mesh.model.NoisePayloadType.VERIFY_RESPONSE -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = com.goatedchat.mesh.model.NoisePayload(
                type = com.goatedchat.mesh.model.NoisePayloadType.DELIVERED,
    ...
    suspend fun handleAnnounce(routed: RoutedPacket): Boolean {
        ...
        if (clockSkewMs > ANNOUNCE_CLOCK_SKEW_TOLERANCE_MS) {
            ...
            return false
        } else if (clockSkewMs > com.goatedchat.mesh.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
        ...
        try {
            val neighborsOrNull = com.goatedchat.mesh.services.meshgraph.GossipTLV.decodeNeighborsFromAnnouncementPayload(packet.payload)
        ...
        try {
            ...
            if (response != null) {
                ...
                val responsePacket = BitchatPacket(
                    ...
                    payload = response,
                    signature = null,
                    ttl = com.goatedchat.mesh.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS
                )
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            val isFileTransfer = com.goatedchat.mesh.protocol.MessageType.fromValue(packet.type) == com.goatedchat.mesh.protocol.MessageType.FILE_TRANSFER
            val file = com.goatedchat.mesh.model.BitchatFilePacket.decode(packet.payload)
            if (file != null) {
                if (isFileTransfer) {
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                com.goatedchat.mesh.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, isFavorite)
                if (npub != null) {
                    // Index by noise key and current mesh peerID for fast Nostr routing
                    com.goatedchat.mesh.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, npub)
                    com.goatedchat.mesh.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, npub)
                }
                ...
                val sys = com.goatedchat.mesh.model.BitchatMessage(
                    sender = "system",
