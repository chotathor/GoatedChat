package com.goatedchat.mesh.onboarding
...
/**
 * Centralized permission management for goatedchat app
...
class PermissionManager(private val context: Context) {
    ...
    companion object {
        ...
        private const val PREFS_NAME = "goatedchat_permissions"
    ...
    private fun shouldRequireWifiAwarePermission(): Boolean {
        ...
        val enabled = try {
            com.goatedchat.mesh.ui.debug.DebugPreferenceManager.getWifiAwareEnabled(false)
        } catch (_: Exception) {
            false
        }
        ...
        return try {
            com.goatedchat.mesh.wifiaware.WifiAwareSupport.isSupported(context)
        } catch (_: Exception) {
            false
        }
    }
    ...
    fun getCategorizedPermissions(): List<PermissionCategory> {
        ...
        categories.add(
            PermissionCategory(
                ...
                description = "Required to discover goatedchat users via Bluetooth",
                permissions = bluetoothPermissions,
                isGranted = bluetoothPermissions.all { isPermissionGranted(it) },
                systemDescription = "Allow goatedchat to connect to nearby devices"
            )
        )
        ...
        categories.add(
            PermissionCategory(
                ...
                description = "Required by Android to discover nearby goatedchat users via Bluetooth",
                permissions = locationPermissions,
                isGranted = locationPermissions.all { isPermissionGranted(it) },
                systemDescription = "goatedchat needs this to scan for nearby devices"
            )
        )
        ...
        if (shouldRequireWifiAwarePermission()) {
            ...
            categories.add(
                PermissionCategory(
                    ...
                    description = "Enable Wi‑Fi Aware to discover and connect to nearby goatedchat users over Wi‑Fi.",
                    permissions = wifiAwarePermissions,
                    isGranted = wifiAwarePermissions.all { isPermissionGranted(it) },
                    systemDescription = "Allow goatedchat to discover nearby Wi‑Fi devices"
                )
            )
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            categories.add(
                PermissionCategory(
                    ...
                    systemDescription = "Allow goatedchat to send you notifications"
                )
            )
        }
        ...
        if (isBatteryOptimizationSupported()) {
            categories.add(
                PermissionCategory(
                    ...
                    description = "Disable battery optimization to ensure goatedchat runs reliably in the background and maintains mesh network connections",
                    permissions = listOf("BATTERY_OPTIMIZATION"), // Custom identifier
                    isGranted = isBatteryOptimizationDisabled(),
                    systemDescription = "Allow goatedchat to run without battery restrictions"
                )
            )
        }
