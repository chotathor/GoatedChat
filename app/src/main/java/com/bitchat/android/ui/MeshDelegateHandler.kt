...
import com.goatedchat.mesh.ui.NotificationTextUtils
import com.goatedchat.mesh.mesh.MeshService
import com.goatedchat.mesh.model.BitchatMessage
import com.goatedchat.mesh.model.DeliveryStatus
...
class MeshDelegateHandler(
                ...
                if (state.getJoinedChannelsValue().contains(message.channel)) {
                    ...
                    val viewingGeohash = try {
                        if (channel.startsWith("geo:")) {
                            ...
                            selected is com.goatedchat.mesh.geohash.ChannelID.Location && selected.channel.geohash.equals(geo, ignoreCase = true)
                        } else false
    ...
    private suspend fun processPeerUpdate(mergedPeers: List<String>) {
        ...
        runCatching { com.goatedchat.mesh.services.MessageRouter.tryGetInstance()?.onPeersUpdated(mergedPeers) }
            ...
            if (isNostrAlias || isNoiseHex) {
                ...
                val canonical = com.goatedchat.mesh.services.ConversationAliasResolver.resolveCanonicalPeerID(
                    ...
                    nostrPubHexForAlias = { alias ->
                        ...
                        if (com.goatedchat.mesh.nostr.GeohashAliasRegistry.contains(alias)) {
                            com.goatedchat.mesh.nostr.GeohashAliasRegistry.get(alias)
                        } else {
                            // Best-effort: derive pub hex from favorites mapping for mesh nostr_ aliases
                            val prefix = alias.removePrefix("nostr_")
                            val favs = try { com.goatedchat.mesh.favorites.FavoritesPersistenceService.shared.getOurFavorites() } catch (_: Exception) { emptyList() }
                            favs.firstNotNullOfOrNull { rel ->
                                rel.peerNostrPublicKey?.let { s ->
                                    runCatching { com.goatedchat.mesh.nostr.Bech32.decode(s) }.getOrNull()?.let { dec ->
                ...
                if (canonical != currentPeer) {
                    ...
                    com.goatedchat.mesh.services.ConversationAliasResolver.unifyChatsIntoPeer(state, canonical, listOf(currentPeer))
                ...
                val favoriteRel = try {
                    ...
                    if (noiseKey != null) {
                        com.goatedchat.mesh.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                    } else null
                ...
                if (favoriteRel?.isMutual == true) {
                    ...
                    if (noiseHex != currentPeer) {
                        com.goatedchat.mesh.services.ConversationAliasResolver.unifyChatsIntoPeer(
                            state = state,
        ...
        mergedPeers.forEach { pid ->
            try {
                ...
                val npub = com.goatedchat.mesh.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKey)
                val tempNostrKey: String? = try {
                    if (npub != null) {
                        val (hrp, data) = com.goatedchat.mesh.nostr.Bech32.decode(npub)
    ...
    private fun unifyChatsIntoPeer(targetPeerID: String, keysToMerge: List<String>) {
        com.goatedchat.mesh.services.ConversationAliasResolver.unifyChatsIntoPeer(state, targetPeerID, keysToMerge)
    }
     ...
     */
    fun getPeerInfo(peerID: String): com.goatedchat.mesh.mesh.PeerInfo? {
