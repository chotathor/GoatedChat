package com.goatedchat.mesh.ui.debug
...
import com.goatedchat.mesh.services.meshgraph.MeshGraphService
...
fun MeshTopologySection(
    ...
    val wifiAwareConnected by com.goatedchat.mesh.wifiaware.WifiAwareController.connectedPeers.collectAsState()
...
fun DebugSettingsSheet(
    ...
    val wifiAwareSupported by com.goatedchat.mesh.wifiaware.WifiAwareController.supported.collectAsState()
    val wifiAwareAvailable by com.goatedchat.mesh.wifiaware.WifiAwareController.available.collectAsState()
    val wifiAwareSupportStatus by com.goatedchat.mesh.wifiaware.WifiAwareController.supportStatus.collectAsState()
        ...
        if (isPresented) {
            ...
            while (true) {
                ...
                val devices = entries.map { (address, isClient, rssi) ->
                    ...
                    com.goatedchat.mesh.ui.debug.ConnectedDevice(
                ...
                try {
                    val ctrl = com.goatedchat.mesh.wifiaware.WifiAwareController
    ...
    BitchatBottomSheet(
        ...
        Box(modifier = Modifier.fillMaxWidth()) {
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        val running by com.goatedchat.mesh.wifiaware.WifiAwareController.running.collectAsState()
                        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                            ...
                            AssistChip(
                                onClick = { com.goatedchat.mesh.wifiaware.WifiAwareController.getService()?.sendBroadcastAnnounce() },
                                enabled = running,
                                label = { Text("Announce") }
                            )
                        }
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        val p = remember(gcsFpr) { com.goatedchat.mesh.sync.GCSFilter.deriveP(gcsFpr / 100.0) }
                        val nmax = remember(gcsFpr, gcsMaxBytes) { com.goatedchat.mesh.sync.GCSFilter.estimateMaxElementsForSize(gcsMaxBytes, p) }
