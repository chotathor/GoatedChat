...
This document specifies the gossip-based synchronization feature for GoatedChat, inspired by Plumtree. It ensures eventual consistency of public packets (ANNOUNCE and broadcast MESSAGE) across nodes via periodic sync requests containing a compact Golomb‑Coded Set (GCS) of recently seen packets.

## Overview

- Each node maintains a rolling set of public GoatedChat packets it has seen recently:
...
Implementation: `com.goatedchat.mesh.sync.PacketIdUtil`.

## GCS Filter (On-demand)

Implementation: `com.goatedchat.mesh.sync.GCSFilter`.
...
- Header: normal GoatedChat header with TTL indicating “local-only” semantics. Implementations SHOULD set TTL=0 to prevent any relay; neighbors still receive the packet over the direct link-layer. For periodic sync, recipient is broadcast; for per-peer initial sync, recipient is the specific peer.
...
Encode/Decode implementation: `com.goatedchat.mesh.model.RequestSyncPacket`.
